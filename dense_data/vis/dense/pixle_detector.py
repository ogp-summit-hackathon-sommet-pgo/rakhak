...
def check_green(image_in_ques):
    ...
    tot_left = str(100 - (float(packed) + float(highway) + float(greennopop) + float(water) + float(road)))
    print(packed, highway, greennopop, water, road, tot_left)
    with open("high_den.csv", 'a') as outfile:
        outfile.write(packed+", "+highway+", "+greennopop+", "+water+", " +road+", "+", "+tot_left+", "+str(-1)+"\n")
